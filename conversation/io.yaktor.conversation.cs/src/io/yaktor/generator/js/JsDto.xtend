...
class JsDto {
        ...
        find: function(query, callback) {
          «IF p.type != null»
            model.find(query, function(err, data) {
              converter.to('«p.fullName»',data,callback);
            });
          «ELSE»
            callback(null, [query]);
          «ENDIF»
  ...
  def map(ProjectionField f){
    f.comments +
    switch(f){
      TypeContainmentField:'''new converter.Field('«f.name»','__«f.name»','«f.eClass.name»',«f.projection.map(f.field)»)'''
      MappedField case f.projection != null:'''new converter.Field('«f.name»','«f.field.name»','«f.field.eClass.name»',«f.projection.map(f.field)»)'''
      ProjectionContainmentField:'''new converter.Field('«f.name»','__«f.name»','«f.eClass.name»',«f.projection.map(f.field)»)'''
      ProjectionField case f.field != null && f.field instanceof TypeField:'''new converter.Field('«f.name»','«f.field.name»','«f.field.eClass.name»',«f.field.map»)'''
      ProjectionField case f.field != null:'''new converter.Field('«f.name»','«f.field.name»','«f.field.eClass.name»',null,«f.field.map»)'''
